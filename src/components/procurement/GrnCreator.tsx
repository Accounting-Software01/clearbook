...
interface SummaryRowProps { label: string; value: string; bold?: boolean; accent?: boolean }
function SummaryRow({ label, value, bold, accent }: SummaryRowProps) {
  return (
    <div className={`flex items-center justify-between py-1.5 ${accent ? 'text-emerald-700 dark:text-emerald-400' : ''}`}>
      ...
      <span className={`font-mono text-sm ${bold ? 'font-bold text-base' : ''}`}>{value}</span>
    </div>
  );
}
...
export function GrnCreator({ poId, onGrnCreated, onCancel }: GrnCreatorProps) {
  ...
  const { subTotal, totalVat, grandTotal, activeLineCount } = useMemo(() => {
    ...
    return { subTotal: sub, totalVat: vat, grandTotal: sub + vat, activeLineCount: active };
  }, [lines]);

  const allFull = lines.every((l) => lineStatus(l) === 'full' || lineStatus(l) === 'pending' && l.quantity === (l.quantity_received ?? 0));
      ...
      <Card className="rounded-b-none border-b-0 border-t-4 border-t-emerald-600 shadow-md">
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-3 border-t bg-muted/30 px-5 py-4 sm:grid-cols-4">
          ...
          <div>
            ...
            <p className="mt-0.5 text-sm">{new Date(order.po_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</p>
          </div>
          <div>
        ...
        <Card className="rounded-none border-t-0 border-x shadow-md lg:border-t lg:border-l-0 lg:w-72 lg:rounded-br-xl lg:rounded-tr-xl">
          ...
          <CardContent className="px-5 pb-4">
            <div className="space-y-0.5">
              ...
              <SummaryRow label="VAT" value={`₦${fmt(totalVat)}`} />

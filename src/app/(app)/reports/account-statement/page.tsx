...
const AccountStatementPage = () => {
    ...
    const generateReport = useCallback(async () => {
        ...
        try {
            ...
            // The API sometimes includes the opening balance as the first transaction
            if (statement.transactions.length > 0 && statement.openingBalance === 0) {
                const firstTx = statement.transactions[0];
                // Assuming the first transaction with an empty description is the opening balance entry
                if (firstTx.description === '' || firstTx.description.toLowerCase() === 'opening balance') {
                    statement.openingBalance = statement.openingBalance + firstTx.debit - firstTx.credit;
                    statement.transactions.shift(); // Remove the first transaction from the list
                }
            }
            ...
            const trendData = statement.transactions.map((tx: Transaction) => {
            ...
            setChartData([
                { date: format(startDate, 'yyyy-MM-dd'), balance: statement.openingBalance },
                ...trendData
            ]);
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    <TableRow className="font-semibold bg-gray-50 dark:bg-gray-800">
                                        <TableCell>{startDate ? format(startDate, 'MMM dd, yyyy') : 'Start of Period'}</TableCell>

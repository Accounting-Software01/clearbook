...
try {
    // This query sums up debits and credits for each account within a date range for a specific company.
    // This is a safer, more standard approach for accounting APIs.
    $sql = "
        SELECT
        ...
        WHERE jv.company_id = ? AND jv.entry_date BETWEEN ? AND ?
    ...
    // Corrected bind_param from "iss" to "sss"
    $stmt->bind_param("sss", $companyId, $fromDate, $toDate);
    $stmt->execute();

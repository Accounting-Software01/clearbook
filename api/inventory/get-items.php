...
function calculate_item_state($item_id, $item_type, $company_id, $conn) {
    ...
    if ($item_type === 'raw_material') {
       ...
       $sql_parts[] = "(SELECT po.completion_date AS date, po.quantity_to_produce AS quantity, po.total_production_cost / NULLIF(po.quantity_to_produce, 0) AS price, po.id AS sort_key FROM production_orders po WHERE po.product_id = ? AND po.company_id = ? AND po.status = 'Completed')";

        array_push($params, $item_id, $company_id);
        $types .= "is";

        // PET Production Output (Stock In)
        $sql_parts[] = "(SELECT ppo.completion_date AS date, ppo.quantity_produced AS quantity, ppo.total_cost / NULLIF(ppo.quantity_produced, 0) AS price, ppo.id AS sort_key FROM pet_production_orders ppo WHERE ppo.product_id = ? AND ppo.company_id = ? AND ppo.status = 'Completed')";
        array_push($params, $item_id, $company_id);
        $types .= "is";
